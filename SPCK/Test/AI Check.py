...
generation_config = {"temperature": 0.5,"top_p": 1,"top_k": 1,"max_output_tokens": 100,}
...
prompt_parts = ["System: LỰA CHỌN ĐÚNG ACTION: Nếu người dùng yêu cầu action liên quan đến Spotify như \"mở nhạc, mở bài {tên bài hát} của {tên nghệ sĩ}, nhạc của {tên nghệ sĩ}, ...\" hãy trả lời \"true - spotify - {từ khóa tìm kiếm ở tiếng Anh}\", không thì trả lời \"false\"", user,]
response = model.generate_content(prompt_parts)
print("AI:",response.text)
prompt_parts = ["System: LỰA CHỌN ĐÚNG ACTION: Nếu người dùng yêu cầu action liên quan đến Youtube như \"mở video, tìm kiếm video,...\" hãy trả lời \"true - youtube - {từ khóa tìm kiếm theo ngôn ngữ của câu hỏi, tránh bắt đầu bằng những từ như \"mở, phát,...\"}\", không thì trả lời \"false\"", user,]
response = model.generate_content(prompt_parts)
print("AI:",response.text)